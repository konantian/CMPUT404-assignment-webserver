...
class MyWebServer(socketserver.BaseRequestHandler):
            ...
            if method == 'GET':
                if self.validPath(request_path):
                    if request_path.endswith('html'):
                        web = open("./www{}".format(request_path),'r').read()
                        header = self.createNormalHeader("HTTP/1.1 200 OK",'text/html',web).encode()
                        self.request.sendall(header)

                    elif request_path.endswith('css'):
                        css = open("./www{}".format(request_path),'r').read()
                        header = self.createNormalHeader("HTTP/1.1 200 OK",'text/css',css).encode()
                        self.request.sendall(header)
                        ...
                        if os.path.isfile(file_path):
                            ...
                            header = self.createNormalHeader("HTTP/1.1 200 OK",'text/html',index_page).encode()
                            self.request.sendall(header)
                        else:
                        ...
                        if os.path.isfile("./www{}/index.html".format(request_path)):
                            ...
                            location = "Location: http://127.0.0.1:8080{}/".format(request_path)
    ...
    def send4XXHeader(self,code):
        ...
        status_code = "HTTP/1.0 404 Not Found" if code == '404' else "HTTP/1.0 405 Method Not Allowed"
    ...
    def createNormalHeader(self,status_code,content_type,content):
        ...
        return "{0}\r\n{1}\r\n{2}\r\n{3}{4}".format(status_code,content_type,content_length,empty_header,content)
    ...
    def validPath(self,path):
        ...
        return os.path.exists(real_path) and os.path.commonpath([current_path,real_path]) == current_path
